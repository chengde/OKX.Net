...
internal class OKXRestClientUnifiedApiExchangeData : IOKXRestClientUnifiedApiExchangeData
{
    ...
    {
        var request = _definitions.GetOrCreate(HttpMethod.Get, $"api/v5/market/open-oracle", OKXExchange.RateLimiter.EndpointGate, 1, false,
    ...
    /// <inheritdoc />
    public virtual async Task<WebCallResult<IEnumerable<OKXBlockTicker>>> GetMarketBlockTickersAsync(InstrumentType instrumentType, string? underlying = null, string? instrumentFamily = null, CancellationToken ct = default)
    {
        var parameters = new ParameterCollection();
        parameters.AddEnum("instType", instrumentType);
        parameters.AddOptionalParameter("uly", underlying);
    ...
    /// <inheritdoc />
    public virtual async Task<WebCallResult<IEnumerable<OKXBlockTrade>>> GetPublicBlockTradesAsync(string symbol, CancellationToken ct = default)
    {
        var parameters = new ParameterCollection
        {
            { "instId", symbol },
